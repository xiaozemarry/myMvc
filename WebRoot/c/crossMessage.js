...
(function () {
    ...
    function jsonToQuery(JSON, isEncode) {
        var _fdata = function (data, isEncode) {
            ...
            if (typeof data == 'object') {
                data = trim(window.JSON.stringify(data));
            } else {
    ...
    var messageHanlder = function (oEvt) {
        ...
        for (var item in args) {
            if (typeof args[item] == 'string' && args[item].indexOf('{') != -1 && args[item].indexOf('}') != -1) {
                args[item] = JSON.parse(args[item]);
            }
        }
        //执行回调
	 ...
	 * @param {Object} oArgs
	 */
    CrossMessage.sendMessage = function (target, oArgs) {
        if (!target) return;
        if (typeof target != 'object') return;
        oArgs["origin"] = window.location.origin;
	 ...
	 */
    CrossMessage.receiveMessage = function (oCallback) {
        callback = oCallback;
    };
})();